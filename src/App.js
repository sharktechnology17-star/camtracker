...
const Btn = ({ children, onClick, bg = "#1e293b", color = "#f1f5f9", style = {} }) => (
  <button onClick={onClick} style={{ background: bg, color, border: "none", borderRadius: 8, padding: "11px 16px", fontWeight: 700, fontSize: 13, cursor: "pointer", fontFamily: "inherit", ...style }}>
    {children}
  </button>
);
    ...
    <div style={{ background: "#0a1628", border: "1px solid #1e293b", borderRadius: 12, padding: 14, marginBottom: 10 }}>
      ...
      <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{ overflow: "visible" }}>
        ...
        {points.map((p, i) => {
          ...
          return (
            <g key={i}>
              <circle cx={p.x} cy={p.y} r={isMin || isMax || isLast ? 5 : 3} fill={c} stroke="#070d1a" strokeWidth="1.5" />
              {(isMin || isMax || isLast) && <text x={p.x} y={p.y - 9} textAnchor="middle" fontSize="8" fill={c} fontFamily="monospace">{fmt(p.costo)}</text>}
            </g>
          );
        })}
      </svg>
...
export default function App() {
          ...
          <div style={{ background: "#0f172a", border: "1px solid #1e293b", borderRadius: 10, padding: 12 }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <div>
                ...
                <div style={{ fontSize: 15, fontWeight: 800, color: "#f1f5f9" }}>{fmt(p.precio_venta)}</div>
              </div>
              <div>
                <div style={{ fontSize: 9, color: "#475569", textTransform: "uppercase" }}>Mínimo recomendado (20%)</div>
                <div style={{ fontSize: 15, fontWeight: 800, color: "#fbbf24" }}>{fmt(precioMinimo)}</div>
              </div>
            </div>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <Inp label="Precio vendido (COP)" type="number" value={formV.precio_venta} onChange={(e) => setFormV({ ...formV, precio_venta: e.target.value })} placeholder={String(p.precio_venta)} />
            ...
            <div style={{ background: "#0a1628", border: `1px solid ${bajoPrecioMin ? "#f87171" : colorM}44`, borderRadius: 10, padding: 14 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: descuento > 0 ? 10 : 0 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 18, fontWeight: 900, color: colorM }}>{margenReal}%</div>
                </div>
              </div>
              {descuento > 0 && (
    ...
    return (
      <div style={ROOT}>
        ...
        <div style={{ ...INNER, maxWidth: 740, paddingTop: 16 }}>
          ...
          <Btn onClick={() => { setSelected(p); resetFormV(); setFormV((f) => ({ ...f, precio_venta: String(p.precio_venta) })); setView("venta"); }} bg="#4ade80" color="#052e16" style={{ width: "100%", padding: 14, fontSize: 15, marginBottom: 10, boxSizing: "border-box" }}>
            💰 Registrar venta
          </Btn>
          ...
          {p.nota && (
            <div style={{ ...CARD, borderColor: "#fbbf2444" }}>
              <div style={{ fontSize: 10, color: "#fbbf24", marginBottom: 4 }}>NOTA</div>
              <div style={{ fontSize: 13, color: "#cbd5e1" }}>{p.nota}</div>
            </div>
          )}
            ...
            <div style={{ background: "#0f172a", border: `1px solid ${cP}44`, borderRadius: 10, padding: 14 }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                <div>
                  ...
                  <div style={{ fontSize: 20, fontWeight: 900, color: "#4ade80" }}>{fmt(gP)}</div>
                </div>
                <div>
                  <div style={{ fontSize: 10, color: "#475569" }}>MARGEN</div>
                  <div style={{ fontSize: 20, fontWeight: 900, color: cP }}>{mP}%</div>
                </div>
              </div>
            </div>
          )}
  ...
  return (
    <div style={ROOT}>
      {toast && (
        <div style={{ position: "fixed", top: 20, left: "50%", transform: "translateX(-50%)", background: toast.type === "err" ? "#7f1d1d" : "#064e3b", color: toast.type === "err" ? "#f87171" : "#4ade80", padding: "10px 20px", borderRadius: 8, fontSize: 13, fontWeight: 700, zIndex: 100, whiteSpace: "nowrap" }}>
          {toast.msg}
        </div>
      )}
      <div style={HDR}>
          ...
          <div style={{ display: "flex", gap: 6 }}>
            {[["inventario", "📦 Inventario"], ["ventas", "💰 Ventas"], ["dashboard", "📊 Dashboard"]].map(([t, l]) => (
              <button key={t} onClick={() => setTab(t)}
                style={{ background: tab === t ? "#0ea5e9" : "#1e293b", color: tab === t ? "#fff" : "#94a3b8", border: "none", borderRadius: 8, padding: "7px 12px", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
                {l}
              </button>
            ))}
          </div>
            ...
            <div style={{ display: "flex", gap: 8, overflowX: "auto", paddingBottom: 4, marginBottom: 10 }}>
              {categories.map((c) => (
                <button key={c} onClick={() => setFilterCat(c)}
                  style={{ background: filterCat === c ? "#0ea5e9" : "#1e293b", color: filterCat === c ? "#fff" : "#94a3b8", border: "none", borderRadius: 8, padding: "7px 13px", fontSize: 12, fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap", fontFamily: "inherit" }}>
                  {c}
                </button>
              ))}
            </div>
