...
const db = {
  async get(t, q="") { const r=await fetch(`${SUPABASE_URL}/rest/v1/${t}?${q}`,{headers:h}); if(!r.ok)throw new Error(await r.text()); return r.json(); },
...
const Tag = ({label,color}) => <span style={{background:color+"22",color,border:`1px solid ${color}44`,borderRadius:4,fontSize:11,padding:"2px 7px",fontWeight:700}}>{label}</span>;
...
const S = {
  ...
  inner: {maxWidth:1100,margin:"0 auto",padding:"0 16px"},
  hdr: {padding:"18px 0 12px",borderBottom:"1px solid #1e293b",background:"#070d1a",position:"sticky",top:0,zIndex:10},
...
const PriceChart = ({historial}) => {
  ...
  const points=historial.map((h,i)=>({x:PAD+(i/(historial.length-1))*(W-PAD*2),y:PAD+((maxVal-h.costo)/range)*(H-PAD*2),...h}));
    ...
    <div style={{background:"#0a1628",border:"1px solid #1e293b",borderRadius:12,padding:14,marginBottom:10}}>
      ...
      <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{overflow:"visible"}}>
        ...
        {points.map((p,i)=>{
          ...
          return <g key={i}><circle cx={p.x} cy={p.y} r={isMin||isMax||isLast?5:3} fill={c} stroke="#070d1a" strokeWidth="1.5"/>{(isMin||isMax||isLast)&&<text x={p.x} y={p.y-9} textAnchor="middle" fontSize="8" fill={c} fontFamily="monospace">{fmt(p.costo).replace(/[^0-9.,]/g,"").replace(".000","k")}</text>}</g>;
        })}
      </svg>
...
export default function App() {
  ...
  const saveVenta=async()=>{
    ...
    const ganancia_total=(precio-p.costo)*cantidad,descuento=(p.precio_venta-precio)*cantidad;
    setSaving(true);
    try{
      await db.post("ventas",{producto_id:p.id,producto_nombre:p.nombre,costo_unitario:p.costo,precio_venta:precio,precio_lista:p.precio_venta,cantidad,ganancia_total,descuento,cliente_nombre:formV.cliente_nombre||null,cliente_telefono:formV.cliente_telefono||null,nota:formV.nota||null,fecha:formV.fecha});
  ...
  const totalUnidades=products.reduce((s,p)=>s+(p.stock||0),0);
  const totalInvertidoAll=totalInvested+ventas.reduce((s,v)=>s+(v.costo_unitario||0)*(v.cantidad||0),0);
  const totalRecuperado=ventas.reduce((s,v)=>s+(v.precio_venta||0)*(v.cantidad||0),0);
  const gananciaReal=ventas.reduce((s,v)=>s+(v.ganancia_total||0),0);
  ...
  if(view==="venta"&&selected){
    ...
    const precioMinimo=Math.ceil(p.costo/(1-0.20));
    ...
    return (
      <div style={S.root}>
        ...
        <div style={{...S.inner,maxWidth:640,display:"flex",flexDirection:"column",gap:14,paddingTop:16}}>
          <div style={{background:"#0f172a",border:"1px solid #1e293b",borderRadius:10,padding:12}}>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              ...
              <div><div style={{fontSize:9,color:"#475569",textTransform:"uppercase"}}>Mínimo recomendado (20%)</div><div style={{fontSize:15,fontWeight:800,color:"#fbbf24"}}>{fmt(precioMinimo)}</div></div>
            </div>
          </div>
          ...
          {formV.precio_venta&&(
            <div style={{background:"#0a1628",border:`1px solid ${bajoPrecioMin?"#f87171":colorM}44`,borderRadius:10,padding:14}}>
              {bajoPrecioMin&&<div style={{background:"#3f0f0f",borderRadius:8,padding:"8px 10px",marginBottom:10,fontSize:11,color:"#f87171"}}>🚨 Por debajo del precio mínimo recomendado ({fmt(precioMinimo)})</div>}
              ...
              {descuento>0&&<div style={{borderTop:"1px solid #1e293b",paddingTop:10}}><div style={{fontSize:9,color:"#475569"}}>DESCUENTO APLICADO</div><div style={{fontSize:14,fontWeight:700,color:"#f87171"}}>-{fmt(descuento)} vs precio lista</div></div>}
            </div>
          )}
  ...
  if(view==="detail"&&selected){
    ...
    return (
      <div style={S.root}>
        ...
        <div style={{...S.inner,maxWidth:740,paddingTop:16}}>
          <div style={{...S.card,borderColor:color+"66"}}>
          ...
          <Btn onClick={()=>{setSelected(p);resetFormV();setFormV(f=>({...f,precio_venta:p.precio_venta.toString()}));setView("venta");}} bg="#4ade80" color="#052e16" style={{width:"100%",padding:14,fontSize:15,marginBottom:10,boxSizing:"border-box"}}>
            💰 Registrar venta
          </Btn>
          ...
          {ventasProducto.length>0&&(
            <div style={S.card}>
              ...
              {ventasProducto.slice(0,5).map((v,i)=>{
                ...
                return (
                  <div key={i} style={{borderBottom:i<Math.min(ventasProducto.length,5)-1?"1px solid #1e293b":"none",paddingBottom:10,marginBottom:10}}>
          ...
          {p.nota&&<div style={{...S.card,borderColor:"#fbbf2444"}}><div style={{fontSize:10,color:"#fbbf24",marginBottom:4}}>NOTA</div><div style={{fontSize:13,color:"#cbd5e1"}}>{p.nota}</div></div>}
          ...
          {loading?<div style={{textAlign:"center",padding:60,color:"#475569"}}>⏳ Cargando...</div>:(
            <>
              ...
              <div style={S.card}>
                ...
                {[["Unidades en stock",totalUnidades,"#f1f5f9"],["Capital invertido total",fmt(totalInvertidoAll),"#94a3b8"],["Ganancia real acumulada",fmt(gananciaReal),"#4ade80"]].map(([l,v,c])=>(
  ...
  return (
    <div style={S.root}>
      ...
      <div style={{...S.inner,paddingTop:14}}>
        {loading?(
          ...
          <>
            <div style={{...S.card,background:"linear-gradient(135deg,#0c1f3a,#0f172a)",borderColor:"#1e3a5f"}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(120px,1fr))",gap:8}}>
                {[["Invertido total",fmt(totalInvertidoAll),"#94a3b8"],["Recuperado",fmt(totalRecuperado),"#38bdf8"],["Ganancia real",fmt(gananciaReal),"#4ade80"]].map(([l,v,c])=>(
                ...
                {filtered.map(p=>{
                  ...
                  return (
                    <div key={p.id} onClick={()=>{setSelected(p);setView("detail");}}
                      style={{...S.card,cursor:"pointer",borderLeft:`3px solid ${color}`,marginBottom:0}}>
